...
def practice_1b():
    ...
    result = re.search(pattern, text)
    if result:
        print(f"Found '2024' at position {result.start()}")
practice_1b()
...
def practice_4c():
    ...
    test_strings = ['\bcat\b', '\bsat\b', '\bmat\b', '\bbat\b']
    pattern = r"\b.at\b"
    for string in test_strings:
        if re.search(pattern,string):
            print(f"Found three-letter word ending in 'at': {string}")
practice_4c()
def practice_5a():
