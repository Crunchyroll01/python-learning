...
for color in colors:
    if re.search(pattern, color):
        print(f"Valid color code: {color}")
'''
practice 1_c
'''
...
for texts in text.split(", "):
    if re.search(pattern, texts):
        print(f"Valid SSN found: {texts}")
    elif re.search(pattern, texts) is None:
        print(f"Invalid SSN found: {texts}")
'''
practice 2_a
'''
...
for date in dates:
    if re.search(pattern, date):
        month, day, year = re.search(pattern, date).groups()
        print(f"Valid date found: {date}")
        print(f"Month: {month}, Day: {day}, Year: {year}")
    else:
        print(f"Invalid date found: {date}")
'''
practice 2_c
'''
...
if match:
    ...
    print(f"Age name: {match.group(2)}")
